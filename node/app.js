...
function passTextToGoogleSearch(senderID, text) {
  const url = `https://www.googleapis.com/customsearch/v1?key=${SEARCH_API_KEY}&cx=${SEARCH_ID}&q=${text}&safe=high`;
  request(url, { json: true }, (err, res, body) => {
    ...
    sendListTemplate(senderID, elements.slice(0,4));
  });
}
...
/*
 * Send a List template message using the Send API.
 *
 */
function sendListTemplate(recipientId, elements) {
  const messageData = {
    recipient: {
      id: recipientId
    },
